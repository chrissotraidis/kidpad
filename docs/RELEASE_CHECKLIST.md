...
| --- | --- | --- |
| Public GitHub source | Technically ready after the checks below | Repository visibility is still private |
| Local unsigned IPA | Ready | The recipient must sign it for their own device |
| Hosted unsigned IPA | Technically ready to package | No GitHub Release has been published |
...
```bash
unzip -tq build/releases/KidPad-unsigned.ipa
unzip -l build/releases/KidPad-unsigned.ipa
(cd build/releases && shasum -a 256 -c KidPad-unsigned.ipa.sha256)
```
...
3. Attach `KidPad-unsigned.ipa` and `KidPad-unsigned.ipa.sha256` only if binary distribution is intentionally approved.
